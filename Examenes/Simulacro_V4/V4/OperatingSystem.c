...
//V2-EX6d-END
//V4-EX6a-BEGIN
int OperatingSystem_BestFittingPartition(int);
//V4-EX6a-END
//V4-EX8-BEGIN
	...
	//V3-EX3-END
		...
		switch (PID) {
			...
			case NOFREEENTRY:
				...
				ComputerSystem_DebugMessage(150,EXAM,programList[i]->executableName);
				programList[i]->arrivalTime+=5;			
				Heap_add(i,arrivalTimeQueue, QUEUE_ARRIVAL, &numberOfProgramsInArrivalTimeQueue);
				//Simulacro-V4
				break;
			//V1-EX4b-END
			//V1-EX5a-BEGIN
...
int OperatingSystem_CreateProcess(int indexOfExecutableProgram) {
	...
	initAddress = partitionsTable[loadingPhysicalAddress].initAddress;
	processTable[PID].numberOfPartition = loadingPhysicalAddress;
	OperatingSystem_ShowTime(SYSMEM);
	ComputerSystem_DebugMessage(143, SYSMEM, loadingPhysicalAddress, initAddress, partitionsTable[loadingPhysicalAddress].size,PID,executableProgram->executableName);
	//V4-EX6c-END
	//V1-EX6a-END
	...
	//V4-EX7-BEGIN
	OperatingSystem_ShowPartitionTable("before_allocating_memory");
	//V4-EX7-END
	...
	//V4-EX7-BEGIN
	OperatingSystem_ShowPartitionTable("after_allocating_memory");
	//V4-EX7-END
...
int OperatingSystem_ObtainMainMemory(int processSize, int PID) {
	...
	//V4-EX6b-END
 	...
 	if (processSize>MAINMEMORYSECTIONSIZE)
		return TOOBIGPROCESS;
	bestFit = OperatingSystem_BestFittingPartition(processSize);	
	if (bestFit==MEMORYFULL){
		return MEMORYFULL;
	} else {
		partitionsTable[bestFit].PID=PID;
		return bestFit;
	} 	
}
//V4-EX6a-END
...
void OperatingSystem_HandleException() {
	...
	switch (Processor_GetRegisterBCPU()){
		case (DIVISIONBYZERO):
			exceptionCase="division by zero";			
			break;		
		case (INVALIDPROCESSORMODE):
			exceptionCase="invalid processor mode";
			break;
		case (INVALIDADDRESS):
			exceptionCase="invalid address";
			break;
		//V4-EX3b-BEGIN
		case (INVALIDINSTRUCTION):
			exceptionCase="invalid instruction";
			break;
		//V4-EX3b-END
	}
	OperatingSystem_ShowTime(INTERRUPT);
	ComputerSystem_DebugMessage(140, INTERRUPT, executingProcessID, programList[processTable[executingProcessID].programListIndex]->executableName, exceptionCase);
	//V4-EX2-END
...
//V4-EX6a-BEGIN
int OperatingSystem_BestFittingPartition(int processSize){
	int bestPartition = MEMORYFULL;
	int currentSize = MAINMEMORYSIZE;
	for (int i=0; i<numberOfPartitions; i++) {
		if (partitionsTable[i].PID==NOPROCESS && partitionsTable[i].size>=processSize) {
			if (partitionsTable[i].size < currentSize) {
				currentSize=partitionsTable[i].size;
				bestPartition=i;
			}
		}
	}
...
void OperatingSystem_ReleaseMainMemory() {
	...
	OperatingSystem_ShowPartitionTable("before_releasing_memory");

	numberOfPartition = processTable[executingProcessID].numberOfPartition;
	initAddress = partitionsTable[numberOfPartition].initAddress;
	size = partitionsTable[numberOfPartition].size;

	OperatingSystem_ShowTime(SYSMEM);
	ComputerSystem_DebugMessage(145, SYSMEM, numberOfPartition, initAddress, size, executingProcessID, programList[processTable[executingProcessID].programListIndex]->executableName);

	partitionsTable[numberOfPartition].PID=NOPROCESS;

	OperatingSystem_ShowPartitionTable("after_releasing_memory");
}
//V4-EX8-END