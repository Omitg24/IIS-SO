...
void OperatingSystem_MoveToTheBLOCKEDState(int PID) {
	if (Heap_add(PID, sleepingProcessesQueue, QUEUE_WAKEUP , &numberOfSleepingProcesses, PROCESSTABLEMAXSIZE)>=0) {		
		...
		processTable[executingProcessID].whenToWakeUp = abs(processTable[executingProcessID].copyOfAcumulatorRegister) + numberOfClockInterrupts + 1;		
		OperatingSystem_ShowTime(SYSPROC);
...
void OperatingSystem_HandleClockInterrupt(){ 
	...
	for (int i=0; i<numberOfSleepingProcesses; i++){
		sleepingProcessWhenToWakeUp = processTable[sleepingProcessesQueue[i].info].whenToWakeUp;
		if (sleepingProcessWhenToWakeUp==numberOfClockInterrupts){				
	...
	if (numberOfUnblockedProcesses>0){
		...
		if (processTable[executingProcessID].priority > processTable[mostPriorityProcessID].priority && mostPriorityProcessID != -1){
			previousProcessID = executingProcessID;
...
int OperatingSystem_GetMostPriorityProcess() {
	...
	// for (int i = 0; i < numberOfReadyToRunProcesses[USERPROCESSQUEUE]; i++){
	// 	currentProcessID = readyToRunQueue[USERPROCESSQUEUE][i].info;
	// 	if (processTable[currentProcessID].priority > processPriority){
	// 		processPriority = processTable[currentProcessID].priority;
	// 		maxProcessID = currentProcessID;
	// 	}
	// }
	// for (int i = 0; i < numberOfReadyToRunProcesses[USERPROCESSQUEUE]; i++){
	// 	currentProcessID = readyToRunQueue[DAEMONSQUEUE][i].info;
	// 	if (processTable[currentProcessID].priority > processPriority){
	// 		processPriority = processTable[currentProcessID].priority;
	// 		maxProcessID = currentProcessID;
	// 	}
